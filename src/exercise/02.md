...
Compound components is a pattern to wire props to the children of a component without exposing them to the user of the component. Using this a component can share state and functions with its children implicitly (without exposing them to the user of component.)

I think we can implement the same thing by using render props, but we would be exposing the state/functions that are shared, and the component user is responsible to do the wiring.

Compound components encapsulate implementation and is simpler to use for the user.
