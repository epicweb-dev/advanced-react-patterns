...
Context Module Functions is a way to extract functionality that call dispatch of the Provider component of React Context, to a function that accepts dispatch as an argument. Normally, we would create the extracted function that call dispatch in the Provider component or the hook that returns the context. It is Ok to do that but has two disadvantages.
1) The functions need to be memoized 
2) The function is coupled with Provider, so cannot be lazy loaded or code splitted.
